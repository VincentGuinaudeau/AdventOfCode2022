...
for (let sensor of sensors) {
	const clear_radius = Math.abs(sensor.x - sensor.beacon.x) + Math.abs(sensor.y - sensor.beacon.y)

	const row_reach = clear_radius - Math.abs(sensor.y - TARGET_ROW)
...
console.log(cleared_ranges.reduce((acc, range) => acc + (range.stop - range.start) + 1, 0) - beacons.filter(beacon => beacon.y === TARGET_ROW).length)
